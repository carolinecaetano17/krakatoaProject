...
public class Compiler {
    ...
    private KraClass classDec() {
        ...
        if ( lexer.token == Symbol.EXTENDS ) {
            ...
            if ( superClass.getName().equals( className ) )
                signalError.show( "Class " + className + " cannot inherit from itself" );
    ...
    private Method methodDec( Type type, String name, String qualifier ) {
        ...
        //Também salvamos o método herdado, caso haja, para comparação de parâmetros
        ...
        while ( superClassHelper != null ) {
            ...
            if ( superClassMethods != null ) {
                for ( Method m : superClassMethods ) {
                    ...
                    } else if ( m.getName().equals( name ) && m.getType() != type ) {
                        signalError.show( "Method " + name + " has a signature different from method inherited from superclass" );
        ...
        //Checa se há retorno no método, incluindo compatibilidade do tipo de retorno
        ...
        //Checa se há break dentro do método
        for ( Statement s : currentMethod.getStatementList() ) {
            if ( s instanceof BreakStatement ) signalError.show( "Break statement outside while" );
        }

        //Se for método herdado, checa compatibilidade de parâmetros
        if ( inheritedMethod != null ) {
            int i = 0;
            ArrayList<Variable> currentMethodParameters = currentMethod.getParamList().getParamList();
            ArrayList<Variable> inheritedMethodParameters = inheritedMethod.getParamList().getParamList();
            while ( i < currentMethodParameters.size() && i < inheritedMethodParameters.size() ) {
                if ( currentMethodParameters.get( i ).getType() == Type.voidType )
                    signalError.show( "Parameters cannot be of type void." );
                else if ( isBasicType( currentMethodParameters.get( i ).getType() ) && currentMethodParameters.get( i ).getType() != inheritedMethodParameters.get( i ).getType() )
                    signalError.show( "Different parameters on inherited method " + currentMethod.getName() + "." );
                else if ( currentMethodParameters.get( i ).getType() instanceof KraClass ) {
                    KraClass currentReturnClass = ( KraClass ) currentMethodParameters.get( i ).getType();
                    KraClass inheritedReturnClass = ( KraClass ) inheritedMethodParameters.get( i ).getType();

                    if ( !currentReturnClass.getName().equals( inheritedReturnClass.getName() ) )
                        signalError.show( "Inherited parameter with different class types on method " + currentMethod.getName() + "." );

                }
                i++;
            }

            if ( i != currentMethodParameters.size() || i != inheritedMethodParameters.size() ) {
                signalError.show( "Method " + currentMethod.getName() + " : received the wrong number of parameters" );
            }
    ...
    private Expr expr() {
        ...
        if ( op == Symbol.EQ || op == Symbol.NEQ || op == Symbol.LE
            ...
            // Checagem semântica para validação de uso de operador lógico
            if ( op == Symbol.LE || op == Symbol.LT || op == Symbol.GE || op == Symbol.GT ) {
                if ( left.getType() != Type.intType || right.getType() != Type.intType )
                    signalError.show( "Trying to use " + op.toString() + " to compare non-integer expressions" );
            } else {
                if ( (left.getType() == Type.intType || left.getType() == Type.booleanType) && left.getType() != right.getType() ) {
                    signalError.show( "Trying to use " + op.toString() + " to compare expressions of different types" );
                } else if ( left.getType() instanceof KraClass && right.getType() instanceof KraClass ) {
                    KraClass leftClass = ( KraClass ) left.getType();
                    KraClass rightClass = ( KraClass ) right.getType();

                    KraClass superClass = rightClass;

                    boolean isSubClass = false;

                    while ( superClass != null && !isSubClass ) {
                        if ( superClass.getName().equals( leftClass.getName() ) )
                            isSubClass = true;
                        superClass = superClass.getSuperclass();
                    }

                    superClass = leftClass;

                    while ( superClass != null && !isSubClass ) {
                        if ( superClass.getName().equals( rightClass.getName() ) )
                            isSubClass = true;
                        superClass = superClass.getSuperclass();
                    }

                    if ( !isSubClass )
                        signalError.show( "Trying to use " + op.toString() + " to compare incompatible class types" );
                }
            }
    ...
    private Expr simpleExpr() {
        ...
        while ( (op = lexer.token) == Symbol.MINUS || op == Symbol.PLUS
            ...
            // Checagem semântica para validação de uso de operador aritmético ou OR
            if ( op == Symbol.MINUS || op == Symbol.PLUS ) {
                if ( left.getType() != Type.intType || right.getType() != Type.intType )
                    signalError.show( "Trying to use " + op.toString() + " on non-integer expressions" );
            } else {
                if ( left.getType() != Type.booleanType || right.getType() != Type.booleanType )
                    signalError.show( "Trying to use " + op.toString() + " on non-boolean expressions" );
            }
    ...
    private Expr term() {
        ...
        while ( (op = lexer.token) == Symbol.DIV || op == Symbol.MULT
            ...
            // Checagem semântica para validação de uso de operador aritmético ou AND
        ...
        switch ( lexer.token ) {
            ...
            case NOT:
                ...
                if ( e.getType() != Type.booleanType )
                    signalError.show( "Trying to use ! on non-boolean expression" );
                        ...
                        if ( lexer.token == Symbol.DOT ) {
                            ...
                            if ( localVar == null ) {
                                ...
                                //Caso contrário, procure pelo método na classe específica
                                    ...
                                    for ( Method m : publicMethods ) {
                                        if ( secondId.equals( m.getName() ) && !m.getQualifier().equals( "private" ) ) {
                                            desiredMethod = m;
                                            desiredClass = calledClass;
                                            methodFound = true;
                                        }
